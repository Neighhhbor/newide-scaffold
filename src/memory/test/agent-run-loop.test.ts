...
describe('Agent persistent loop (runLoopTick)', () => {
  describe('assignTask', () => {
    ...
    it('Pipeline 模式的 assignTask 仅设 state，不设 loopMessages', async () => {
      ...
      agent.assignTask(createTestTask());
    ...
    it('已有任务时 assignTask 抛出错误', async () => {
      ...
      agent.assignTask(createTestTask());
      expect(() => {
        agent.assignTask(createTestTask({ task_id: 'task_002' }));
      }).toThrow('already has a running task');
    });
...
describe('AgentManager tickAll', () => {
  it('驱动所有 running agent 走一步', async () => {
    ...
    agentA.assignTask(createTestTask({ task_id: 'task_a' }));
    agentB.assignTask(createTestTask({ task_id: 'task_b' }));
  ...
  it('只 tick running agent，忽略 idle/stopped', async () => {
    ...
    runningAgent.assignTask(createTestTask());
