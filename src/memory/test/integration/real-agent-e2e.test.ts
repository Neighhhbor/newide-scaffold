 ...
 */
function createRealDriver(label: string): InvokeDriverTool {
...
suite('E2E 真实 Agent 循环 (Agent → Driver → Buffer → Extract → PG → 第二轮)', () => {
  ...
  beforeAll(async () => {
    ...
    embedding = hasEmbeddingKey ? new LiteLLMEmbeddingProvider() : new HashEmbeddingProvider();
    ...
    const driverTool = createRealDriver('Cycle1');
  ...
  it('Cycle 2: 新 Agent 实例执行第二轮任务，载入第一轮经验', async () => {
    ...
    // 新 Driver（真实 CLI Driver）
    const driverTool2 = createRealDriver('Cycle2');
